...
class Pet(Base):
    ...
    litter_id = Column(Integer, ForeignKey("litters.id"), nullable=True, comment="所属同窝ID")
    is_neutered = Column(Boolean, default=False, comment="是否绝育")
    price = Column(Integer, nullable=True, comment="售价（分），仅 role=在售 时有意义")
    is_for_sale = Column(Boolean, default=False, comment="是否在售中")
